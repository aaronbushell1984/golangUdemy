...
Cover only documents the use of cover for reference.

To call cover on all pacakges from root:

	go test ./... -cover

To write coverage out to a file named c.out:

	go test ./... -coverprofile c.out

To launch a created coverage file named c.out in a browser:

	go tool cover -html=c.out

Running the help testglag command (which is found as a addiotional topic) under go test:
